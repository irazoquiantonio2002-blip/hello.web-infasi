document.addEventListener('DOMContentLoaded', () => {
    ...
    if (form) {
        form.addEventListener('submit', e => {
            ...
            let txt = `Hola KHOR Seguridad Privada, soy *${name}*.\n`;
            ...
            setTimeout(() => {
                window.open(`https://wa.me/525566167998?text=${encodeURIComponent(txt)}`, '_blank');
                form.reset();
...
function initTypewriter() {
    ...
    if (!el) return;
    // All words agree grammatically: "que [word] lo que Más Importa"
    const words = ['Protege', 'Cuida', 'Resguarda', 'Vigila', 'Blinda', 'Custodia'];
